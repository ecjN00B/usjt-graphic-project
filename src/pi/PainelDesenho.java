...
public class PainelDesenho extends JPanel
    ...
    public void doDesenho(Figura figura, Graphics2D gg) {
        ...
        ArrayList<Integer> coordenadas = figura.getCoordenadas();
        Point[] desenho;
        if (coordenadas != null) {
            desenho = new Point[coordenadas.size() + 1];
            desenho[0] = ponto.getLocation();

            for (int i = 0; i < coordenadas.size(); i++) {
                switch (coordenadas.get(i)) {
                    case 0:
        ...
        if (figura.equals(aux_figura)) {
            if (tipo_desenho == Desenho.FREEMAN && mousemoved) {
                if (aux_figura.getCoordenadas() != null) {
                    aux_ponto = desenho[aux_figura.getCoordenadas().size()].getLocation();
                } else {
    ...
    public void mouseDragged(MouseEvent e) {
        if (tipo_desenho == Desenho.LIVRE && e.getModifiersEx() != 4096) {
            ...
            aux_figura.addCoordenadas(aux_cord);
            repaint();
        }
